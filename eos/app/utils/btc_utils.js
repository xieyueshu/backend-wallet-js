...
module.exports = {
	...
	getFees: (txn) => {
		let totalIn = txn.vin.reduce((total, vin) => total + vin.value, 0);
	...
	getBlockTxns: async (start, end, detail = false) => {
		...
		logger.debug("btc_utils.getBlockTxns - Found " + txnHashList.length + "BTC transactions");
		return txnHashList
	},
	getBlock: (hash) => {
		const client = getClient();
	...
	getTxn: (hash) => {
		const client = getClient();
		return client.getRawTransaction(hash, 1);
	},
	...
	createBtcWithdrawRequest: async (request, db, after) => {
		after({
			"msg": "BTC withdrawal to be created"
		});
	},
	...
	createBtcWallet: async (res, db, walletType = "BTC") => {
		let params = {
			network: process.env.NODE_ENV === "production" ? bitcoin.networks.bitcoin : bitcoin.networks.testnet
		};
		...
		logger.info("BTC Address generated: " + keyPair.getAddress());
		if(process.env.BTC_INSIGHT_SUPPORT === "N") {
			const client = getClient();
			await client.command("importaddress", address.address, "", false);
			logger.debug("btc_utils.createBtcWallet - imported address " + address.address);
		}
		db.collection('address').insert(address, (err, item) => {
			if (err) {
				...
				logger.info("Sending to client: " + JSON.stringify(address));
				res.send(address);
			}
		});
	},
