...
module.exports = {
	...
	getTransactions: async (account, position, offset, eosSettings) => {
		const response = await axios.get(process.env.EOS_EXPLORER_API_URL, {
			params: {
		...
		if (response.status == 200) {
			if (response.data.actions.length > 0) {
				let txns = _.filter(response.data.actions, o => {
					...
					(o.action_trace.act.account == eosSettings.contract || o.action_trace.act.data.quantity.includes(eosSettings.coinName)) &&
					o.action_trace.receiver == account;
				}).map(cur => {
					return _.merge(cur.action_trace.act.data, {
						accountActSeq: cur.account_action_seq,
