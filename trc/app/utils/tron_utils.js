...
module.exports = {
  fromRawContractAmt,
  addressFromHex: constants.tronWeb ? constants.tronWeb.address.fromHex : null,
  ...
  getTokenBalance: async (wallet) => {
    ...
    const balances = acct.withPriceTokens || acct.trc20token_balances;
  ...
  getLastBlockNum: async () => {
    ...
    return blockNum - parseInt(process.env.TRX_BLOCK_RETRIEVAL_COUNT || process.env.TRX_CONFIRMATION_COUNT);
  },
  transferToCold: async(db, unsentList) => {
	...
	logger.debug('returned from getTransferData is ' + walletSend);
        if(walletSend){
	  logger.debug('Sending tron transaction : ' + JSON.stringify(walletSend)); 
  ...
  getEventForBlock: async(blockNum, url = null) => {
    let config = {};
    if(!url) {
      url = `${process.env.TRX_TRONGRID_API_URL}/v1/contracts/${process.env.TRX_CONTRACT_ADDRESS}/events`;
      config = {
        params: { block_number:blockNum, limit: 200 }
      };
    }
    ...
    if(resData.success) {
      const eventList = resData.data;
      // no next page, so return the list
      if(!resData.meta.links)  return eventList;

      let nextLink = resData.meta.links.next;
	  if(!nextLink.toLowerCase().startsWith(process.env.TRX_TRONGRID_API_URL.toLowerCase())) {
		  nextLink = process.env.TRX_TRONGRID_API_URL + nextLink;
	  }
      const nextList = await module.exports.getEventForBlock(blockNum, nextLink);
      return eventList.concat(nextList);
    } else {
      throw new Error("Unsuccessful retrieval of contract events");
    }
  }
};
