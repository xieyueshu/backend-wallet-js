...
module.exports = {
  ...
  send: async (db, wallet, callback, willAdd = true) => {
    ...
    let gasLimit = web3.toHex(parseInt(process.env.ETH_GAS_LIMIT));
    ...
    let details = {
      ...
      gas: gasLimit, chainId: web3.toHex(process.env.ETH_CHAIN_ID)
    };
  ...
  collectDepositWallet: async (db, web3, depositWallet) => {
    ...
    if (depositWallet.type === "ETH") {
      ...
      if(new BigNumber(depositWallet.amount).lt(process.env.ETH_GAS_AMTC_MIN || 0)){ 
		...
		return;
      } else if (ethSend.getRawAmount(depositWallet.amount).gt(Number.MAX_SAFE_INTEGER)) { 
        logger.debug(`deposit_collection_job - ${depositWallet.address} amount exceeds approve amount`);
		return;
	  }
  ...
  transferToColdErc: async (addr, db, web3, amount = null) => {
    ...
    if(!(await ethDb.isApprovedAddress(db, hotWallet, addr.address))) {
      await approveAddress(db, web3, hotWallet, addr);
    } else {
  ...
  checkLog: async (web3, walletList, txn) => {
    ...
    if(!DEFI_CONTRACT_LIST.includes(txn.to) && txn.input.toLowerCase().search(contractNoPrefix) === -1) return;
...
const approveAddress = async (db, web3, spender, addr) => {
