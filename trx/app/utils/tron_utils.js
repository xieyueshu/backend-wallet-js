...
module.exports = {
  ...
  send: async (db, wallet) => {
    ...
    let txnId = ""; 
    if(wallet.type === "TRX"){ 
      logger.debug('sending TRX' + wallet.amount);
      const txn = await tronSend.getTxnData(wallet, constants.tronWeb);
      logger.debug('done getting txn data' +  JSON.stringify(txn));
      txnId = txn.txID;
      const signedTxn = await constants.tronWeb.trx.sign(txn,wallet.key);
      logger.debug('done singing txn');
      await constants.tronWeb.trx.sendRawTransaction(signedTxn); 
      logger.debug('done sending raw transaction  ');
    } else {
      logger.debug('sending token ' + wallet.type);
