...
const returnWithdrawRequest = (res, details)  => {
  if(Object.hasOwnProperty.call(details,"requestId")) {
...
module.exports = function(app, db) {
  ...
  app.get("/getWithdrawStats", async (req, res) => {
    try{
      ...
      const txnStats = await db.collection("transaction").mapReduce(function() {
        ...
        const reducedVal = { count: 0, amount: 0 };
  ...
  app.post("/withdraw", async (req, res)=>{
    ...
    if(req.body.type){
      ...
      else if(chainList.includes("TRX") && tronUtil.isTronType(type)){
        tronUtil.createWithdrawRequest(req.body, db, function(details){
          returnWithdrawRequest(res, details);
        });			
      }
      else if(type=="FIL" && chainList.includes("FIL")){
        filUtil.createWithdrawRequest(req.body, db, function(details){
          returnWithdrawRequest(res, details);
        });			
      }
      else {
  ...
  app.post("/rejectWithdrawal",[isLoggedIn, hasPermissionAction], async (req, res) => {
    try{
      ...
      for (const req of dbReq) {
        req.approvedStatus = "R";
        await httpUtil.sendRequest(req, db);
      }
...
const getWithdrawTxns = async (db, coinType, after) => {
  ...
  txns.forEach(() => {
    ...
    if(txns[cnt].coinType === "TRC20") txns[cnt].coinType = process.env.TRX_CONTRACT_SYMBOL;
    cnt++;
  });
...
const getWithdrawRequests = async (db, coinType, dbquery) => {
  ...
  //Nov 12 2020(zhen) - added checking to apply only to ETH and LTK
  let web3;
  if (coinType=="ETH"){
    web3 = new Web3(ethUtil.WEB3_PROVIDER);
  }
  ...
  requests.forEach((request) => {
    if(coinType === "AMTC"){
      ...
      //Nov 12 2020(zhen) - added checking to apply only to ETH and LTK
    } else if (coinType === "ETH" || coinType=="LTK"){
