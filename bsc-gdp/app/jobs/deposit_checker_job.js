...
var child = null, trxBlockProcess = {child: null, skipCount: 0};
var isRunning = false;
...
const retrieveEthTxns = async (db, isBind) => {
  ...
  for (var i = fromBlk.value; i < toBlk; i++) {
    ...
    if(!block){ 
      logger.warn("No block data found for block " + i);
      continue;
    }
    ...
    for (let i = 0; i < txnHashList.length; i++) {
      ...
      if (to) {
        ...
        if (contractAddr === to) {
            ...
            if (result.inputs[0]) {
              ...
              if (amtcAddr.indexOf(to) !== -1) {
                ...
                if (receipt.status === "0x1") {
                  ...
                  if(process.env.ETH_CONTRACT_DEPOSIT_CHECK_LOGS === "Y") {
          ...
          const txns = await ethUtil.checkLog(web3, amtcAddr, txn, true);
          await processEthLogs(txns, txn, db, rates, markedAddr);
        }
      }
    }
...
const createAndSendTxn = async (db, txn, det) => {
  ...
  if (det.index && det.index>0) {
    txnDetails.index=det.index;
    txnDetails.indexHash=txn.hash+"["+det.index+"]";
  }
  if (det.rate) {
...
const killBlockProcess = () => {
  trxBlockProcess.child.kill();
  delete trxBlockProcess.child;
};
...
const retrieveTrxTxns = async(db) => {
  if(!trxBlockProcess.child) {
    trxBlockProcess.child = cp.fork("app/processes/trx_block_process.js");
    trxBlockProcess.skipCount = 0;
    let timeobj = setTimeout(()=>{
      if (trxBlockProcess.child){
        logger.info("deposit_checker_job - force-killing block process child due to inactivity");
        killBlockProcess();
      }
    }, process.env.TRX_BLOCK_TIMEOUT*1000);
		
    trxBlockProcess.child.on("error",function(err) {
      logger.error("deposit_checker_job - block process returned error : "+ err);
      killBlockProcess();
      clearTimeout(timeobj);
    });

    trxBlockProcess.child.on("message", function(data) {
      logger.debug("deposit_checker_job - Received data from trx_block_process: " + data);
      killBlockProcess();
      clearTimeout(timeobj);
    });
  }
  ...
  const blockList = await db.collection("trx_block").find({status:{$nin:["DONE","PROCESSING"]}}).sort({block:1}).limit(TRX_QUEUE_SIZE).toArray();
