...
const getBlock = async (blockNum) => {
  ...
  if(apiUse === "tronscan") {
    ...
    while(res.data.meta.links) {
      ...
      if (!nextUrl.toLowerCase().startsWith(process.env.TRX_TRONGRID_API_URL.toLowerCase())){
...
const getLastBlock = async () => {
  const data = await axios.get(process.env.TRX_TRONSCAN_API_URL + "/block/latest", { timeout: 10000 });
