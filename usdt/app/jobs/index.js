...
const depositCollectJob = require("./deposit_collection_job");
...
const settings = require("../utils/setting_utils");
...
MongoClient.connect(db.url, async (err, database) => {
  ...
  await settings.loadSystemWallets(walletDb);
  ...
  if (process.env.RUN_DEPOSIT_JOB === "Y") {
    ...
    depositCollectJob.start(walletDb);
  }
