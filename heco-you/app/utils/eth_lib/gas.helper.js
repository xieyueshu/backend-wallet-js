    ...
    try {
      if(process.env.ETH_OTHER_CHAIN_SUPPORT === "Y") {
        if(process.env.ETH_OTHER_CHAIN_NAME === "HECO") {
          logger.info("sendHelper - retrieving gas price from tc.hecochain");
          let response = await axios.get("https://tc.hecochain.com/price/prediction", { timeout: 15000 });
          price = response.data.prices.fast;
        } else {
          logger.info("sendHelper - retrieving gas price with util for chain " + process.env.ETH_OTHER_CHAIN_NAME);
          const web3Eth = new Web3Eth(process.env.NODE_ETH_NETWORK);
          const web3 = new Web3(process.env.NODE_ETH_NETWORK);
          const gasResponse = await web3Eth.getGasPrice();
          price = web3.fromWei(gasResponse, "gwei");
        }
      } else {
